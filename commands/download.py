...
from constants import BACK, LOGIN, PROCESSING, STORIES
...
from constants.messages import (DOWNLOAD_COMPLETED, GETTING_STORY_INFORMATION,
                                IS_VIDEO, LINK_IS_INVALID,
                                OK_SEND_ME_THE_LINK_YOU_WANT_TO_DOWNLOAD,
                                SOMETHING_WENT_WRONG, STARTING_DOWNLOAD,
                                UPLOAD_IN_TELEGRAM,
...
async def download(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if not logged_in_user:
        await update.message.reply_text(
            text=SOMETHING_WENT_WRONG, reply_markup=base_keyboard
        )
        return HOME_STATE
    if message_is_url:
        ...
        if STORIES in url_slices:
            ...
            try:
                ...
                getting_media_information = await context.bot.send_message(
                    chat_id=update.message.chat_id, text="Getting media information ..."
                )
        ...
        if media_type == PHOTO:
            ...
            getting_story_information = await context.bot.send_message(
                chat_id=update.message.chat_id, text=GETTING_STORY_INFORMATION
            )
